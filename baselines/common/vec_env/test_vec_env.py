...
def assert_venvs_equal(venv1, venv2, num_steps):
    ...
    try:
        (obs1, _), (obs2, _) = venv1.reset(), venv2.reset()
        ...
        for _ in range(num_steps):
            ...
            assert len(outs1) == 5, len(outs1)
            assert len(outs2) == 5, len(outs2)
            for out1, out2 in zip(outs1[:4], outs2[:4]):
                assert np.array(out1).shape == np.array(out2).shape
                assert np.allclose(out1, out2)
            assert list(outs1[4]) == list(outs2[4])
    finally:
        venv1.close()
        venv2.close()
...
class SimpleEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self._cur_step += 1
        terminated = self._cur_step >= self._max_steps
        reward = self._cur_step / self._max_steps
        return self._cur_obs, reward, terminated, False, {'foo': 'bar' + str(reward)}
